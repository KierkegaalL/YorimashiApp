...
**設計フェーズ完了、実装フェーズ開始**(2026-07-18に`develop`ブランチを作成)。詳細設計8件はすべて確定済みで、Notion正本の一括更新(A1+B)、FR-8/FR-9の削除・FR-15(会話ペイン)の追加、開発用Live2Dモデルの実配置(A2)も完了。**未決事項は8件**残っているが、いずれも実装着手をブロックしない。
