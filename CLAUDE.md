...
├── Memory.md                  # セッション間の状況記録(完了/未決事項14件/技術情報)
├── docs/
...
**設計フェーズ完了。実装は未着手**(スキャフォールドのみ)。詳細設計7件はすべて確定済みだが、**未決事項が14件**残っており、うち2件(`baseResolution`の形式間非対称、開発用Live2Dモデルの不在)は実装着手をブロックする。
