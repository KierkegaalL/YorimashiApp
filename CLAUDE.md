...
├── Memory.md                  # セッション間の状況記録(完了/未決事項の決着記録/技術情報)
...
9. **マージ先は `develop`**
   実装着手にともない `develop` を作成済み(2026-07-18)。**以降のマージ先は常に `develop`** とする(決定済み)。詳細は [.claude/rules/git-workflow.md](.claude/rules/git-workflow.md)。コミット・プッシュは**ユーザーの指示があったときのみ**行う。
...
**設計フェーズ完了、実装フェーズ開始**(2026-07-18に`develop`ブランチを作成)。詳細設計8件はすべて確定済みで、Notion正本の一括更新(A1+B)、FR-8/FR-9の削除・FR-15(会話ペイン)の追加、開発用Live2Dモデルの実配置(A2)も完了。**未決事項(A/B/C系)は2026-07-29に全件決着した**(最後がC0=Notion/Obsidian連携のv1スコープ外化)。
