...
1. **ハーネス整備の続き**（ユーザーから追加指示の予定。対称性フックの改善案は下記）
2. A2（Live2Dモデル配置）→ A1+B一括Notion更新 → 実装着手
3. Cは各機能の実装時に回収

### ハーネス整備で認識済みの課題（未着手）

**対称性フック（`symmetry-reminder.sh`）の誤検知が多い。** 原因は3つ:

1. **ファイルの中身しか見ていない**（変更内容を見ていない）。CLAUDE.mdのルールは「**変更をしたら**両方確認する」なので、`git diff`の変更行を見る実装にすべき
2. **`EMOTION_STATES`をスプライトセット専用マーカーにしている** — 実際は両形式の単一の情報源であり、これが最大の誤り
3. **Zodのenum記法 `z.enum(['live2d','spriteset'])` に一致しない**／**ドキュメントは散文の「Live2D」で書かれる**ため検出パターンに引っかからない

実際の誤検知例: `emotions.ts`（形式非依存なのに「スプライトセットのみ」）、`config-schema.ts`（対称なのに「Live2Dのみ」）、`lipsync.md`（Live2D 5回言及なのに「スプライトセットのみ」）、`data.md`（2.1/2.2両方あるのに「Live2Dのみ」）。
