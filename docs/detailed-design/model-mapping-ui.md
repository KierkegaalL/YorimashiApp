...
| スプライトセットの差し替え | 「変更」= ネイティブダイアログ + 行へのドロップの**両対応**。削除はインライン確認 |
...
- **表示幅は`maxWidth: 400`**(モックアップの実値)。この幅の制約が論点2の結論を決めている。
...
モックアップの表示幅は`maxWidth: 400`。`Row`はラベルを左、コントロールを右に置くレイアウトなので、**右側に残る幅は180px程度しかなく、2つのselectを横並びにすると読めない**。
...
#### 「変更」ボタンの挙動: ダイアログとドロップの両対応

モックアップの追加フローは「mp4/webmをドロップ」と**ドロップで統一**されている(L696)。一方で差し替えは`Upload`アイコンの**ボタン**(L826)なので、クリックに対する応答が必要になる。片方だけにすると、追加時に覚えた操作が差し替え時に通じない。

- **クリック**: `dialog.showOpenDialog`をmp4/webmフィルタつきで開く
- **行へのドロップ**: 同じ取り込み処理へ流す(追加フローと同じ操作感)

取り込み後の処理はspriteset-pipeline.mdで確定済みのパイプライン(Chromiumでデコード → 境界連結判定で色キー抜き → sharpでアニメーションWebPへ)をそのまま通す。差し替えと新規追加で経路を分けない。
