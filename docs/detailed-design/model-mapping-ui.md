...
- **表示幅は`maxWidth: 400`**。Chrome拡張のサイドパネル(FR-8)に収まる単一カラムとして設計されている。この幅の制約が論点2の結論を決めている。
...
- **FR-9の表示排他制御により、キャラクターウィンドウは「非表示」の可能性がある**。ブラウザ拡張側が表示権を持っている間、Electronのキャラクターウィンドウは`hide()`されている。この状態でプレビューを再生しても何も起きず、ユーザーには操作が壊れたようにしか見えない。
