...
>
> **決着(2026-07-30、実機報告のユーザー要望対応時)**: 上記の「未決」を、**Rendererが初回ロード後に実サイズを報告して書き戻す**案で実装した(Cubism Core導入は取り込み時=Main側の話であり、現状Coreをバンドルしていない制約は変わらないため見送り)。`Live2DRenderer.loadModel()`が初回描画直後に`internalModel.width/height`(実測値)をIPC(`CharacterReportLive2dSize`)でMainへ送り、`character-window.ts`の`normalizeLive2dBaseResolution()`がアスペクト比を実測どおりに補正した`baseResolution`へ書き換えてから`applyDisplaySize()`でウィンドウへ反映する(モデル切替直後の古い報告は`appliedModelId`との一致判定で無視し、既に補正済みなら再計算結果が同値になるため無用な書き込み・リサイズは起きない)。あわせて「表示サイズ100%でもキャラクターが小さく見える」というユーザー要望に応えるため、正規化の目標長辺を`FALLBACK_BASE_RESOLUTION`の2倍(`LIVE2D_BASE_RESOLUTION_TARGET_MAX`)にしている。**Renderer側の描画スケール(`fitModel()`)に倍率を掛ける設計は採らなかった**: `containScale`に定数を掛けるとcontainの定義上必ずどこかの辺がウィンドウ外へはみ出す(実機確認で「上下が枠外にはみ出す」不具合として再現)。`baseResolution`(=ウィンドウの物理サイズ)自体を大きくすれば`fitModel()`は純粋なcontainのままで済み、はみ出しが構造的に起きない。
