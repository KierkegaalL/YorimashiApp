...
> **実装(#10)**: このJSONは手書きの複製を持たず、`src/main/code-adapter/hooks-settings.ts` が `shared/hook-events.ts` の `HOOK_EVENT_NAMES` から生成する(`Record<HookEventName, …>` にしてあるため、イベントを増やすとテンプレート側が型エラーになり、受信側だけ実装して案内に出し忘れる事故を防ぐ)。**アプリはこのファイルを書き込まない**——利用者の既存hooks設定を破壊しうるため、オンボーディングはクリップボードへのコピーまでしか行わない(onboarding.md 論点3)。配置するのは新規ファイルである `dispatch.sh` だけで、その書き込み先も**ネイティブダイアログで利用者が選んだディレクトリ**に限定する。
