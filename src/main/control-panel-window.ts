 ...
 *
 * 実測(Electronオフスクリーン。'Zen Antique', serif 19px。ConversationPane.tsx の実スタイルと同じ条件):
 *   - 「霊力状態 ── 静穏(せいおん)」の幅 = 252.89px(confident/tiredとも同一。Moodラベルはいずれも
 *     「漢字2字+ひらがな4字」で文字数が揃っており、和文フォントは字ごとの送り幅が均一なため
 *     3種とも同じ幅になる。これはバグではなく和文タイポグラフィの性質)
 *   - 全角1文字の幅 = 19px(≒1em。「1.5文字」は 28.5px として扱う)
 *
 * **実測条件が本番と一致することの確認(2026-07-28 再実測)**: 当初(2026-07-20)は Google Fonts を
 * 同梱しない方針だったため 'Zen Antique' は本番でも総称 `serif` へフォールバックする前提で
 * 実測し、392.59px→393pxとしていた。**その後、未決事項C2の決着で `@fontsource/zen-antique` を
 * ローカル同梱した**(src/renderer/control-panel/src/fonts.css)ため、本番では実際に 'Zen Antique'
 * が解決されるようになり、上記の前提が崩れた。そのため実際にビルドされたフォントアセット
 * (out/renderer/assets/)を読み込んだ状態で再実測した(推測で据え置かない。constraints.md)。
 * 結果は252.89px(旧値252.59pxから+0.3px)で、最終的な丸め結果(下記計算)は**393pxのまま変化なし**
 * だったため、定数自体の変更は不要と判断した。
 ...
 *   36(左右padding) + 1(border) + 60(avatar) + 14(gap) + 252.89(テキスト) + 28.5(1.5文字)
 *   ≈ 392.4px → 切り上げて 393px。
 *
