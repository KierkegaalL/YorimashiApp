...
export const ModelSlotSchema = z.object({
  ...
  baseResolution: z.object({ width: z.number(), height: z.number() }), // 形式共通・必須。spritesetはmanifest.jsonから読む。live2dは取り込み時に暫定でフォールバック(400×400)を入れる(model3.jsonにキャンバス寸法が無く、moc3の正確な寸法はCubism Coreが要ると実測で判明。model-importer.ts参照。正確な取得は未決)
